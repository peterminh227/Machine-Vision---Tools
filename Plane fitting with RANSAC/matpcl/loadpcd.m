...
function points = loadpcd(fname)
    ...
    points = cell2struct(cell(numFields, 1), fields);
    ...
    switch mode
        case 'ascii'
            ...
            points = [];
            for j=1:length(c)
                % if the type is unsigned int, trying to pack it with
                % floats in the same matrix will round the floats. Using
                % single seems to strip bits from the rgba representation
                % so we use double instead.                
                points = [points; double(c{j}')];
            end
        ...
        case 'binary'
            ...
            if all(types == types(1)) && all(sizes == sizes(1))
                ...
                for i=1:numFields
                    ...
                    switch fields{i}
                        case 'x'
                            points(1,:) = double(data);
                        case 'y'
                            points(2,:) = double(data);
                        case 'z'
                            points(3,:) = double(data);
                        case 'rgb'
                            rgb = uint32(data);
                        ...
                        case 'rgba'
                            rgb = uint32(data);
                            A = double(bitand(255, bitshift(rgb, -24))) /255;
                            R = double(bitand(255, bitshift(rgb, -16))) /255;
                            G = double(bitand(255, bitshift(rgb, -8))) /255;
                            B = double(bitand(255, rgb)) /255;
                            points = [points(1:3,:); R; G; B; A];
                            points(:,randperm(size(points,2), 5))
                    end
                end
        ...
        case 'binary_compressed'
            ...
            for i=1:numFields
                ...
                switch fields{i}
                    case 'x'
                        points(1,:) = double(field);
                    case 'y'
                        points(2,:) = double(field);
                    case 'z'
                        points(3,:) = double(field);
                    case {'rgb', 'rgba'}
                        points(4,:) = double(field);
                end
            end           
    ...
    % some of the loads do not convert the 4th channel, others do. If there
    % are 4 rows, this means that it's not done yet
    if size(points,1) == 4
